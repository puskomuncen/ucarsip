 ...
 * Page class
 */
class TracksEdit extends Tracks
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "TracksEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "tracksedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("track_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->track_id->setQueryStringValue($keyValue);
                $this->track_id->setOldValue($this->track_id->QueryStringValue);
            } elseif (Post("track_id") !== null) {
                $this->track_id->setFormValue(Post("track_id"));
                $this->track_id->setOldValue($this->track_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("track_id") ?? Route("track_id")) !== null) {
                    ...
                    $this->track_id->CurrentValue = null;
                }
            }
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'track_id' before field var 'x_track_id'
        $val = $this->getFormValue("track_id", null) ?? $this->getFormValue("x_track_id", null);
        if (!$this->track_id->IsDetailKey) {
            $this->track_id->setFormValue($val);
        }
    ...
    {
        $this->track_id->CurrentValue = $this->track_id->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // track_id
            $this->track_id->setupEditAttributes();
            $this->track_id->EditValue = $this->track_id->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->track_id->Visible && $this->track_id->Required) {
                if (!$this->track_id->IsDetailKey && IsEmpty($this->track_id->FormValue)) {
                    $this->track_id->addErrorMessage(str_replace("%s", $this->track_id->caption(), $this->track_id->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // action
        $this->_action->setDbValueDef($newRow, $this->_action->CurrentValue, $this->_action->ReadOnly);

        // keterangan
        $this->keterangan->setDbValueDef($newRow, $this->keterangan->CurrentValue, $this->keterangan->ReadOnly);
