...
class UnitsEdit extends Units
{
    ...
    // Page object name
    public string $PageObjName = "UnitsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "unitsedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("unit_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->unit_id->setQueryStringValue($keyValue);
                $this->unit_id->setOldValue($this->unit_id->QueryStringValue);
            } elseif (Post("unit_id") !== null) {
                $this->unit_id->setFormValue(Post("unit_id"));
                $this->unit_id->setOldValue($this->unit_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("unit_id") ?? Route("unit_id")) !== null) {
                    $this->unit_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("unitslist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "unitslist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "unitslist") {
                            ...
                            $returnUrl = "unitslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'unit_id' before field var 'x_unit_id'
        $val = $this->getFormValue("unit_id", null) ?? $this->getFormValue("x_unit_id", null);
        if (!$this->unit_id->IsDetailKey) {
            $this->unit_id->setFormValue($val);
        }

        // Check field name 'nama_unit' before field var 'x_nama_unit'
        $val = $this->getFormValue("nama_unit", null) ?? $this->getFormValue("x_nama_unit", null);
        if (!$this->nama_unit->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nama_unit->Visible = false; // Disable update for API request
            } else {
                $this->nama_unit->setFormValue($val);
            }
        }

        // Check field name 'kode_unit' before field var 'x_kode_unit'
        $val = $this->getFormValue("kode_unit", null) ?? $this->getFormValue("x_kode_unit", null);
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->nama_unit->CurrentValue = $this->nama_unit->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        // nama_unit
        $this->nama_unit->RowCssClass = "row";

        // kode_unit
        $this->kode_unit->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // nama_unit
            $this->nama_unit->setupEditAttributes();
            $this->nama_unit->EditValue = !$this->nama_unit->Raw ? HtmlDecode($this->nama_unit->CurrentValue) : $this->nama_unit->CurrentValue;
            $this->nama_unit->PlaceHolder = RemoveHtml($this->nama_unit->caption());

            // kode_unit
            $this->kode_unit->setupEditAttributes();
            $this->kode_unit->EditValue = !$this->kode_unit->Raw ? HtmlDecode($this->kode_unit->CurrentValue) : $this->kode_unit->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->unit_id->Visible && $this->unit_id->Required) {
                if (!$this->unit_id->IsDetailKey && IsEmpty($this->unit_id->FormValue)) {
                    $this->unit_id->addErrorMessage(str_replace("%s", $this->unit_id->caption(), $this->unit_id->RequiredErrorMessage));
                }
            }
            if ($this->nama_unit->Visible && $this->nama_unit->Required) {
                if (!$this->nama_unit->IsDetailKey && IsEmpty($this->nama_unit->FormValue)) {
                    $this->nama_unit->addErrorMessage(str_replace("%s", $this->nama_unit->caption(), $this->nama_unit->RequiredErrorMessage));
                }
            }
            if ($this->kode_unit->Visible && $this->kode_unit->Required) {
                if (!$this->kode_unit->IsDetailKey && IsEmpty($this->kode_unit->FormValue)) {
                    $this->kode_unit->addErrorMessage(str_replace("%s", $this->kode_unit->caption(), $this->kode_unit->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow(): bool
    {
        ...
        // Check field with unique index (kode_unit)
        if ($this->kode_unit->CurrentValue != "") {
            $filterChk = "(`kode_unit` = '" . AdjustSql($this->kode_unit->CurrentValue) . "')";
            ...
            if ($rsChk->fetchAssociative()) {
                $idxErrMsg = sprintf($this->language->phrase("DuplicateIndex"), $this->kode_unit->CurrentValue, $this->kode_unit->caption());
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // nama_unit
        $this->nama_unit->setDbValueDef($newRow, $this->nama_unit->CurrentValue, $this->nama_unit->ReadOnly);

        // kode_unit
        $this->kode_unit->setDbValueDef($newRow, $this->kode_unit->CurrentValue, $this->kode_unit->ReadOnly);
    ...
    {
        if (isset($row['nama_unit'])) { // nama_unit
            $this->nama_unit->CurrentValue = $row['nama_unit'];
        }
        if (isset($row['kode_unit'])) { // kode_unit
            $this->kode_unit->CurrentValue = $row['kode_unit'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("unitslist"), "", $this->TableVar, true);
        $pageId = "edit";
