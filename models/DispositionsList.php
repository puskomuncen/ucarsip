 ...
 * Page class
 */
class DispositionsList extends Dispositions
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "DispositionsList";
    ...
    public string $FormName = "fdispositionslist";

    // CSS class/style
    public string $CurrentPageName = "dispositionslist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "dispositionsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "dispositionsdelete";
        $this->MultiUpdateUrl = "dispositionsupdate";
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fdispositionsgrid";
        }
		...
		if (ReadCookie('dispositions_searchpanel') == 'notactive' || ReadCookie('dispositions_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('dispositions_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fdispositionssrch");
        }
        $filterList = Concat($filterList, $this->disposition_id->AdvancedSearch->toJson(), ","); // Field disposition_id
        $filterList = Concat($filterList, $this->letter_id->AdvancedSearch->toJson(), ","); // Field letter_id
        $filterList = Concat($filterList, $this->dari_unit_id->AdvancedSearch->toJson(), ","); // Field dari_unit_id
        $filterList = Concat($filterList, $this->ke_unit_id->AdvancedSearch->toJson(), ","); // Field ke_unit_id
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fdispositionssrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->disposition_id->AdvancedSearch->SearchValue = $filter["x_disposition_id"] ?? "";
        $this->disposition_id->AdvancedSearch->SearchOperator = $filter["z_disposition_id"] ?? "";
        $this->disposition_id->AdvancedSearch->SearchCondition = $filter["v_disposition_id"] ?? "";
        $this->disposition_id->AdvancedSearch->SearchValue2 = $filter["y_disposition_id"] ?? "";
        $this->disposition_id->AdvancedSearch->SearchOperator2 = $filter["w_disposition_id"] ?? "";
        $this->disposition_id->AdvancedSearch->save();
        ...
        // Field dari_unit_id
        $this->dari_unit_id->AdvancedSearch->SearchValue = $filter["x_dari_unit_id"] ?? "";
        $this->dari_unit_id->AdvancedSearch->SearchOperator = $filter["z_dari_unit_id"] ?? "";
        $this->dari_unit_id->AdvancedSearch->SearchCondition = $filter["v_dari_unit_id"] ?? "";
        $this->dari_unit_id->AdvancedSearch->SearchValue2 = $filter["y_dari_unit_id"] ?? "";
        $this->dari_unit_id->AdvancedSearch->SearchOperator2 = $filter["w_dari_unit_id"] ?? "";
        $this->dari_unit_id->AdvancedSearch->save();

        // Field ke_unit_id
        $this->ke_unit_id->AdvancedSearch->SearchValue = $filter["x_ke_unit_id"] ?? "";
        $this->ke_unit_id->AdvancedSearch->SearchOperator = $filter["z_ke_unit_id"] ?? "";
        $this->ke_unit_id->AdvancedSearch->SearchCondition = $filter["v_ke_unit_id"] ?? "";
        $this->ke_unit_id->AdvancedSearch->SearchValue2 = $filter["y_ke_unit_id"] ?? "";
        $this->ke_unit_id->AdvancedSearch->SearchOperator2 = $filter["w_ke_unit_id"] ?? "";
        $this->ke_unit_id->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->disposition_id); // disposition_id
            $this->updateSort($this->letter_id); // letter_id
            $this->updateSort($this->dari_unit_id); // dari_unit_id
            $this->updateSort($this->ke_unit_id); // ke_unit_id
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->disposition_id->setSort("");
                $this->letter_id->setSort("");
                $this->dari_unit_id->setSort("");
                $this->ke_unit_id->setSort("");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"dispositions\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"dispositions\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"dispositions\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdispositionslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdispositionslist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->disposition_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"dispositions\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "disposition_id");
            $this->createColumnOption($option, "letter_id");
            $this->createColumnOption($option, "dari_unit_id");
            $this->createColumnOption($option, "ke_unit_id");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdispositionssrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdispositionssrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdispositionslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_dispositions", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_dispositions",
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('dispositions_searchpanel') == 'notactive' || ReadCookie('dispositions_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdispositionssrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('dispositions_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdispositionssrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdispositionssrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
