 ...
 * Page class
 */
class TracksView extends Tracks
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "TracksView";
    ...
    // CSS class/style
    public string $CurrentPageName = "tracksview";
...
// Is modal
        ...
        if (($keyValue = Get("track_id") ?? Route("track_id")) !== null) {
            ...
            $this->RecKey["track_id"] = $this->track_id->QueryStringValue;
        } elseif (Post("track_id") !== null) {
            $this->track_id->setFormValue(Post("track_id"));
            $this->RecKey["track_id"] = $this->track_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->track_id->setQueryStringValue($keyValue);
            $this->RecKey["track_id"] = $this->track_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "trackslist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("trackslist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->track_id->CurrentValue, $this->CurrentRow['track_id'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "trackslist"; // No matching record, return to list
                    } else {
