...
/**
 * Table class for tracks
 */
class Tracks extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $track_id;
    public DbField $letter_id;
    public DbField $user_id;
    public DbField $_action;
    public DbField $keterangan;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "tracks"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // track_id
        $this->track_id = new DbField(
            $this, // Table
            'x_track_id', // Variable name
            'track_id', // Name
            '`track_id`', // Expression
            '`track_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`track_id`', // Virtual expression
            false, // Is virtual
        ...
        $this->track_id->InputTextType = "text";
        $this->track_id->Raw = true;
        $this->track_id->IsAutoIncrement = true; // Autoincrement field
        $this->track_id->IsPrimaryKey = true; // Primary key field
        $this->track_id->Nullable = false; // NOT NULL field
        $this->track_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->track_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['track_id'] = &$this->track_id;
        ...
        $this->_action->Raw = true;
        $this->_action->Nullable = false; // NOT NULL field
        $this->_action->Required = true; // Required field
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
        ...
        $this->_action->OptionCount = 4;
        $this->_action->SearchOperators = ["=", "<>"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tracks";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->track_id->setDbValue($conn->lastInsertId());
            $row['track_id'] = $this->track_id->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['track_id']) && !IsEmpty($this->track_id->CurrentValue)) {
                $row['track_id'] = $this->track_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('track_id', $row)) {
                AddFilter($where, QuotedName('track_id', $this->Dbid) . '=' . QuotedValue($row['track_id'], $this->track_id->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->track_id->DbValue = $row['track_id'];
    ...
    {
        $keys = [];
        $val = $current ? $this->track_id->CurrentValue : $this->track_id->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->track_id->CurrentValue = $keys[0];
            } else {
                $this->track_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('track_id', $row) ? $row['track_id'] : null;
        } else {
            $val = !IsEmpty($this->track_id->OldValue) && !$current ? $this->track_id->OldValue : $this->track_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@track_id@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("trackslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "tracksview" => $this->language->phrase("View"),
            "tracksedit" => $this->language->phrase("Edit"),
            "tracksadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TracksView",
            Config("API_ADD_ACTION") => "TracksAdd",
            Config("API_EDIT_ACTION") => "TracksEdit",
            Config("API_DELETE_ACTION") => "TracksDelete",
            Config("API_LIST_ACTION") => "TracksList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("tracksview", $parm);
        } else {
            $url = $this->keyUrl("tracksview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tracksedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("trackslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tracksadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("trackslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("tracksdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"track_id\":" . VarToJson($this->track_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->track_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->track_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("track_id") ?? Route("track_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->track_id->CurrentValue = $key;
            } else {
                $this->track_id->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->track_id);
