...
(function($) {
    ...
    var VariableListWidget = PhpDebugBar.Widgets.VariableListWidget = KVListWidget.extend({
        ...
        itemRenderer: function(dt, dd, key, value) {
            ...
            if (v && v.length > PhpDebugBar.MAX_CHARS) {
                v = v.substr(0, PhpDebugBar.MAX_CHARS) + "...";
            }
            var prettyVal = null;
        ...
        render: function() {
            ...
            this.$list = new ListWidget({ itemRenderer: function(li, value) {
                if (value.message_html) {
                    ...
                    var m = value.message;
                    if (m.length > PhpDebugBar.MAX_CHARS) {
                        m = m.substr(0, PhpDebugBar.MAX_CHARS) + "...";
                    }

                    var val = $('<span />').addClass(csscls('value')).text(m).appendTo(li);
                    if (!value.is_string || value.message.length > PhpDebugBar.MAX_CHARS) {
