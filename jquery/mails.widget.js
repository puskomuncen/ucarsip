    ...
    var MailsWidget = PhpDebugBar.Widgets.MailsWidget = PhpDebugBar.Widget.extend({
        ...
        render: function() {
            this.$list = new  PhpDebugBar.Widgets.ListWidget({ itemRenderer: function(li, mail) {
                    ...
                    if (mail.body) {
                        var mailBody = $('<pre />').addClass(csscls('body')).appendTo(li);
                        $('<code />').text(mail.body).appendTo(mailBody);
                        li.click(function() {
                            if (mailBody.is(':visible')) {
                                mailBody.hide();
                            } else {
                                mailBody.show();
                            }
                    ...
                    if (mail.html) {
                        var mailHtml = $('<pre />').addClass(csscls('html')).appendTo(li);
                        $('<code />').html(hljs.highlight(mail.html, {language: 'html'}).value).appendTo(mailHtml);
                        li.click(function() {
                            if (mailHtml.is(':visible')) {
                                mailHtml.hide();
                            } else {
                                mailHtml.show();
                            }
