...
<form name="flettersadd" id="flettersadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script<?= Nonce() ?>>
loadjs.ready(["flettersadd", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['flettersadd', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("flettersadd", "x_tanggal_surat", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['flettersadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("flettersadd", "x_tanggal_terima", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_letters_penerima_unit_id">
    <select
        ...
        data-select2-id="flettersadd_x_penerima_unit_id"
        <?php } ?>
        data-table="letters"
...
<?php if (!$Page->penerima_unit_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("flettersadd", function() {
    var options = { name: "x_penerima_unit_id", selectId: "flettersadd_x_penerima_unit_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (flettersadd.lists.penerima_unit_id?.lookupOptions.length) {
        options.data = { id: "x_penerima_unit_id", form: "flettersadd" };
    } else {
        options.ajax = { id: "x_penerima_unit_id", form: "flettersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_file_url" id= "fa_x_file_url" value="0">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="flettersadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="flettersadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#flettersadd:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
