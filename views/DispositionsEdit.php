...
<form name="fdispositionsedit" id="fdispositionsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { dispositions: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fdispositionsedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdispositionsedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["disposition_id", [fields.disposition_id.visible && fields.disposition_id.required ? ew.Validators.required(fields.disposition_id.caption) : null], fields.disposition_id.isInvalid],
...
<?php if ($Page->disposition_id->Visible) { // disposition_id ?>
    <div id="r_disposition_id"<?= $Page->disposition_id->rowAttributes() ?>>
        <label id="elh_dispositions_disposition_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->disposition_id->caption() ?><?= $Page->disposition_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->disposition_id->cellAttributes() ?>>
<span id="el_dispositions_disposition_id">
<span<?= $Page->disposition_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->disposition_id->getDisplayValue($Page->disposition_id->getEditValue()))) ?>"></span>
<input type="hidden" data-table="dispositions" data-field="x_disposition_id" data-hidden="1" name="x_disposition_id" id="x_disposition_id" value="<?= HtmlEncode($Page->disposition_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_dispositions_letter_id">
    <select
        ...
        data-select2-id="fdispositionsedit_x_letter_id"
        <?php } ?>
...
<?php if (!$Page->letter_id->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fdispositionsedit", function() {
    var options = { name: "x_letter_id", selectId: "fdispositionsedit_x_letter_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdispositionsedit.lists.letter_id?.lookupOptions.length) {
        options.data = { id: "x_letter_id", form: "fdispositionsedit" };
    } else {
        options.ajax = { id: "x_letter_id", form: "fdispositionsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script<?= Nonce() ?>>
loadjs.ready("fdispositionsedit", function() {
    fdispositionsedit.createAutoSuggest(Object.assign({"id":"x_dari_unit_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->dari_unit_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.dispositions.fields.dari_unit_id.autoSuggestOptions));
});
</script>
<?= $Page->dari_unit_id->Lookup->getParamTag($Page, "p_x_dari_unit_id") ?>
<script<?= Nonce() ?>>
loadjs.ready(['fdispositionsedit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fdispositionsedit", "x_dari_unit_id", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<script<?= Nonce() ?>>
loadjs.ready("fdispositionsedit", function() {
    fdispositionsedit.createAutoSuggest(Object.assign({"id":"x_ke_unit_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->ke_unit_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.dispositions.fields.ke_unit_id.autoSuggestOptions));
});
</script>
...
loadjs.ready(['fdispositionsedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fdispositionsedit", "x_ke_unit_id", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdispositionsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdispositionsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
